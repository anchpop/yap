...
//! into sentences, living at `generate-data/data/<lang>/sentence-sources/books/<book>.jsonl`.
//!
//! The files are produced by `src/bin/translate_book.rs` (LLM translation of chunks from
//! an English source book, segmented by the parsley `/segment` serve) and are consumed by
//! [`crate::target_sentences::get_target_sentences`] like any other sentence source, so
//! book sentences flow into the gold labeling (clean-nlp-data) and everything downstream.
