...
import { type Deck, type Weapon, type Language, type GramDictionaryEntry } from '../../../yap-frontend-rs/pkg'
    ...
    <div className="flex-1 overflow-hidden flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-2">
        <div className="space-y-4">
          {entries.map((entry) => {
            const morphologyText = entry.morphology ? formatMorphology(entry.morphology) : ''
            ...
            <Card key={entry.frequency_index} className="p-4 relative gap-0">
              ...
              <div className="space-y-3">
                {'Dictionary' in entry.definition ? (
                  entry.definition.Dictionary.definitions.map((def, defIndex) => (
                  ...
                  <div className="pl-4 border-l-2 border-muted">
                    <div className="font-medium text-primary">{entry.definition.Phrasebook.meaning}</div>
                    {(entry.definition.Phrasebook.target_language_example || entry.definition.Phrasebook.native_language_example) && (
                      <div className="mt-2 text-sm space-y-1">
                        ...
                        {entry.definition.Phrasebook.native_language_example && (
                          <div className="text-muted-foreground">
                            <span>{nativeLangCode}:</span> {entry.definition.Phrasebook.native_language_example}
                          </div>
                        )}
                      </div>
                    )}
